 ...
 * 作用：
 *
 * ConfigurableListableBeanFactory 和 BeanDefinitionRegistry 的默认实现，
 * 一个基于bean定义元数据的完整bean工厂，可通过后处理器扩展。
 * Spring's default implementation of the {@link ConfigurableListableBeanFactory}
 * and {@link BeanDefinitionRegistry} interfaces: a full-fledged bean factory
 * based on bean definition metadata, extensible through post-processors.
 *
 * 在访问 bean 前，首先注册所有的 bean definition。
 * 因此，在本地 bean definition 表中按名查找 Bean 是廉价操作，对预解析的bean定义元数据对象进行操作
...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	private String[] doGetBeanNamesForType(ResolvableType type, boolean includeNonSingletons, boolean allowEagerInit) {
		...
		for (String beanName : this.beanDefinitionNames) {
			...
			// 不根据别名进行查找
			// 因为alias和name的bean是一致的，省略重复查找
			if (!isAlias(beanName)) {
				try {
					RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);

					// 只检查完整的 beanDefinition
					// 不是抽象类 且
					//   允许提前初始化  或
					//     存在BeanClass 或 非懒加载 或 允许早类加载
					//     早需要在初始化
					if (!mbd.isAbstract() && (allowEagerInit ||
						...
						// 允许提前初始化 || 不是FactoryBean || mdd非懒加载 || 存在对应单例
						// 且
						// isTypeMatch
						boolean matchFound =
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		// 获取之前缓存的 bean definition
		BeanDefinition existingDefinition = this.beanDefinitionMap.get(beanName);

		// 之前已经创建同 beanName 的 bean definition
		if (existingDefinition != null) {
			// 不允许覆盖，报错
			if (!isAllowBeanDefinitionOverriding()) {
				throw new BeanDefinitionOverrideException(beanName, beanDefinition, existingDefinition);
			}
			// 允许则打印相关日志，并使用 beanName 注册 bean definition
		...
		// 第一次创建该 beanName 的 bean definition
		else {
	...
	/**
	 * 通过DependencyDescriptor和beanName解析依赖bean、
	 * @param descriptor
	 * @param beanName
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			// 处理 Stream、Array、Collection和Map
	...
	/**
	 * 解析存在多个bean的 bean：Stream、Array、集合类和Map
	 *
	 * @param descriptor
	 * @param beanName
	 * @param autowiredBeanNames
	...
	protected Map<String, Object> findAutowireCandidates(
		...
		// 根据 requiredType 获取候选的 beanNeme
		String[] candidateNames = BeanFactoryUtils.beanNamesForTypeIncludingAncestors(
				this, requiredType, true, descriptor.isEager());

		// 从缓存中加载 candidateNames 对应的 bean instance
